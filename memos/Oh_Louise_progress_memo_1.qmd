...
## Potential Prediction Problem 1

::: {.callout-note icon="false"}
## Prediction Research Objective 1
...
## Potential Prediction Problem 2

::: {.callout-note icon="false"}
## Prediction Research Objective 2
...
The raw data has 16 variables and 10772 observations. There are 6 character variables and 10 numeric variables. There is a large number of missing data in all variables except `OSF_ID` and `Language`. For variables with less number of missing values, missing values may be handled during making the recipes for my model. However, for the purpose of this progress memo and data check, data cleaning was performed where all observations with missing values were removed.
...
After removing all missing values, there are 5156 observations, one unique ID and 15 variables. There are 9 numerical variables (maternal_age, postnatal_depression, promis_anxiety, gestational_age, birth_length, birth_weight, threaten_life, threaten_baby_danger, threaten_baby_harm), 5 categorical variables (household_income, maternal_education, delivery_mode, nicu_stay, language), and one date variable (delivery_date). The dataset meets the minimum guidelines of sufficiently complex for this particular project even after the data cleaning. 
...
Here is the data codebook that shows the variables and their descriptions. 
...
A possible concern is that if a large number of observations are filtered out due to missingness issues, the dataset may be no longer original and the prediction outcome may potentially be misleading and distorted from its original intentions. If missing values are addressed using imputation in creating the recipe, challenges might arise, like bias, loss of variability, increased uncertainty, model overfitting, etc. Another way of addressing is excluding variables with large missing values when making the models, if feasible. However, this issue must be approached cautiously, and the limitations of the  data should be acknowledged in any subsequent analysis or interpretation.
