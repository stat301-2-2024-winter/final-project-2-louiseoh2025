...
-   Elastic Net: Lasso Regression (glmnet engine):
    ...
    -   The penalty was explored over \[ , \] with 10 levels.
...
-   K Nearest Neighbor (kknn engine):
    ...
    -   The neighbors was explored over \[25, 40\] with 15 levels.
...
The lasso regression, ridge regression, k nearest neighbor, boosted tree, and random forest models had their parameters tuned. Sub-models and the tuning process for each model types were examined. First, @fig-autoplots-better is a visual inspection of autoplots. The models are evaluated based on their RMSE, which was previously selected as the comparison metric. @fig-autoplots-better only shows the plots for each model type using the better performing recipe. See @fig-autoplots for RMSE autoplots for all models (See Appendix: EDA).
...
It is easier to view the best hyperparameters, using RMSE for comparison, on each of the objects containing the tuning information. @tbl-parameters shows the best hyperparameters of each model adjusted to and used to fit the data for both recipes.
