...
The COVID-19 pandemic was a substantial stressor, especially for pregnant individuals. The Mental Health In Pregnancy During COVID-19 Dataset[^1] is a dataset used in a research study aimed to understand the impact of COVID-19-related stresses on pregnant women and their infants and collected survey-based data across Canada as part of the Pregnancy during the COVID-19 Pandemic (PdP) project (Lebel et al., 2023)[^2]. This survey-based data allows for insights that can inform targeted interventions and support strategies to address the impact of COVID-19-related stresses on maternal and infant health.
...
After performing data cleaning, there are 6078 observations with 18 variables. There is one target variable, birth weight, 8 factor predictors and 9 numeric predictors. There are missing values in all variables excluding the target variable, unique ID, and the language survey was answered in. However, all variables have over 90% complete rate.
...
## Model Building & Selection Methods

#### Data Splitting

The cleaned pregnancy data was split into training (80%) and testing (20%) datasets. The split was stratified by the target variable, birth weight. There are 6078 observations in the cleaned dataset, thus 4861 observations in the training dataset and 1217 observations in the testing dataset. 
...
The following basic steps were performed in creating the kitchen sink recipe:
...
Recipe for tree-based model was one-hot dummy encoded. Recipe for tree-based model is different based on how the models inherently handle categorical variables. Tree-based models benefit such encoding to properly utilize categorical variables in their numerical framework.

**Customized Recipe (recipe 2)**
...
-   Apply square root transformation to Edinburgh Postnatal Depression Scale for normality.

    -   Before the predictors were normalized, the Edinburgh Postnatal Depression Scale data was transformed for a normal distribution. This decision was based on a univariate analysis shown in @fig-appendix-eda1 (see Appendix: EDA). 

-   Add interaction term between Patient-Reported Outcomes Measurement Information System (PROMIS) on Anxiety and Edinburgh Postnatal Depression Scale.

    -   Adding an interaction term between anxiety and depression levels allows the models to capture nuanced relationships that cannot be fully explained by considering each variable independently. This decision is backed up by secondary research that showed evidence that anxiety, oftentimes, may occur as a symptom of clinical depression. Similarly, it is common to have depression that is triggered by an anxiety disorder, including general, panic, and separation anxiety disorders. Many people have a diagnosis of both an anxiety disorder and clinical depression (Kalin, 2020)[^3]. A bivariate analysis in @fig-appendix-eda2 also showed that these two variables had a strong linear correlation with a correlation coefficient of around 0.8 (see Appendix: EDA).

Recipe for tree-based model had interaction terms removed. Interaction terms are often unnecessary and potentially detrimental when using tree-based models. Focusing on simpler feature representations allows tree-based models to leverage their strengths in capturing complex interactions while maintaining interpretability and generalization performance. Recipe for tree-based model was one-hot dummy encoded, just like the kitchen sink recipe. 

[^3]: Lebel, C., Tomfohr-Madsen, L., Giesbrecht, G., Lai, B. P. Y., Bagshawe, M., Freeman, M., Hapin, M. K., MacKinnon, A., Patel, P., van Sloten, M., & van de Wouw, M. (2023). Prenatal mental health data and birth outcomes in the pregnancy during the COVID-19 Pandemic dataset. Data in brief, 49, 109366. --- <https://doi.org/10.1016/j.dib.2023.109366>

#### Model Type and Parameter Tuning

The Null model was used as a baseline model. A Linear Regression model was fitted using resampling. Lasso Regression, Ridge Regression, K Nearest Neighbor, Boosted Tree, and Random Forest models were tuned and fitted. The Null model used the kitchen sink recipe for parametric models and all other models were fitted using both the kitchen sink recipe and the customized recipe, which were both modified for parametric and tree-based models.
...
-   Elastic Net: Lasso Regression (glmnet engine):
    ...
    -   The mixture was set to 1 for lasso regression.
    -   The penalty was explored over with 5 levels.

-   Elastic Net: Ridge Regression (glmnet engine):

    -   The mixture was set to 0 for ridge regression.
    -   The penalty was explored over with 5 levels.

-   K Nearest Neighbor (kknn engine):

    -   The neighbors was explored over \[40, 80\] with 15 levels.

-   Boosted Tree (xgboost engine):

    -   The number of trees was set to 1000, a standard number for tree-based models.
    -   The number of randomly selected predictors to split on was explored over \[2, 25\], since there were 43 predictors in the recipes used.
    -   The minimum number of data points in a node for splitting was explored over \[2, 25\].
    -   The learning rate was explored over with 5 levels.

-   Random Forest (ranger engine):

    -   The number of trees was set to 1000, a standard number for tree-based models.
    -   The number of randomly selected predictors to split on was explored over \[2, 30\], since there were 43 predictors in the recipes used. The maximum was set slightly higher compared to the Boosted Tree since the best performing submodel using \[2, 25\] had relatively higher values of predictors used.
    -   The minimum number of data points in a node for splitting was explored over \[2, 30\].
...
RMSE, MAE, RSQ, and MAPE are used to assess the chosen final model. These four metrics in combination offer a comprehensive evaluation of the model's performance from various perspectives. RMSE (Root Mean Squared Error) measures the average magnitude of the errors between predicted and observed values, with lower values indicating better performance. MAE (Mean Absolute Error) measures the average absolute differences between predicted and observed values, providing a direct measure of the magnitude of errors without considering their direction. RSQ (R-Squared) represents the proportion of the variance in the dependent variable that is predictable from the independent variables in a regression model, with higher values indicating better fit. MAPE (Mean Absolute Percentage Error) measures the average percentage difference between predicted and observed values, providing a relative measure of the accuracy of predictions. RMSE and MAE provide insights into the magnitude of errors, while MAPE offers a relative measure of accuracy, and RSQ indicates how well the model explains the variance in the dependent variable, collectively providing a holistic understanding of the model's predictive capability, accuracy, and goodness of fit.

## Model Building & Selection Results

#### Comparing Submodels

The Lasso Regression, Ridge Regression, K Nearest Neighbor, Boosted Tree, and Random Forest models had their parameters tuned. Submodels and the tuning process for each model types were examined. First, a visual inspection of autoplots was done over each model using the better-performing recipe. (See Appendix: RMSE by Model Parameters for autoplots of all models using the worse-performing recipe)

@fig-autoplot-en1 shows the RMSE by parameters for the Elastic Net models (Lasso and Ridge Regressions) using the kitchen sink recipe. The larger the amount of regularization, the better the RMSE was. However, the difference was large for Lasso Regression and minimal for Ridge Regression. 

```{r}
#| label: fig-autoplot-en1
#| fig-cap: "RMSE by Hyperparameters of Elastic Net (recipe 1) Models"
#| echo: false

(lasso1_auto + ridge1_auto)
```

@fig-autoplot-knn2 shows the RMSE by parameters for the K Nearest Neighbor models using the customized recipe. This plot shows that when the number of nearest neighbors is between 55 and 60, the RMSE will be at its lowest and perform the best.

```{r}
#| label: fig-autoplot-knn2
#| fig-cap: "RMSE by Hyperparameters of K Nearest Neighbor (recipe 2) Model"
#| echo: false

knn2_auto
```

@fig-autoplot-tree1 shows the RMSE by parameters for the other Boosted Tree model using the kitchen sink recipe. The submodel with a learning rate of 0.316 performed significantly better than other submodels. Within that learning rate, the rough trend seemed to be that the larger the number of randomly selected predictors, the lower the RMSE. 

```{r}
#| label: fig-autoplot-tree1
#| fig-cap: "RMSE by Hyperparameters of Boosted Tree (recipe 1) Model"
#| echo: false

bt1_auto
```

@fig-autoplot-tree2 shows the RMSE by parameters for the other Random Forest model using the kitchen sink recipe. The graph shows that, for all minimal node sizes in general, the RMSE decreases as the number of randomly selected predictors increases. However beyond around 20 predictors, the RMSE seems to slightly increase for submodels with smaller number of minimal node sizes. 

```{r}
#| label: fig-autoplot-tree2
#| fig-cap: "RMSE by Hyperparameters of Random Forest (recipe 1) Model"
#| echo: false

rf1_auto
```

The autoplots for tree-based models are better explained in the next section using a table of the best hyperparameters by RMSE for comparison on each of the objects containing the tuning information. @tbl-parameters shows the best hyperparameters of the tree models adjusted to and used to fit the data for both recipes. These tables were used after fitting each model to adjust the parameters based on the best performing submodel for each model type. 

```{r}
#| label: tbl-parameters
#| tbl-cap: "Table of Best Hyperparameters for Each Model"
#| echo: false
...
For the K Nearest Neighbor models, the best performing submodel was when the number of neighbors was set to 60 and 57 resepectively for recipe 1 and 2. For the Boosted Tree models using both recipes, the submodels that performed best had 19 predictors that were randomly sampled at each split when creating the models, 13 minimum data points in a node that are required for the node to be split further, and a learning rate of approximately 0.316. For the Random Forest models using both recipes, the submodels that performed best had 19 predictors that were randomly sampled at each split when creating the models and 13 minimum data points in a node that are required for the node to be split further. 

#### Selecting A Model: RMSE 

@tbl-analysis presents the RMSE (Root Mean Square Error) values for different models created with both recipes, serving as indicators of predictive accuracy. Among the listed models, the Random Forest model using the kitchen sink recipe stands out, boasting the lowest RMSE of 399.23. On average, the birth weight predictions made by the Random Forest model using recipe 1 are approximately 399.23 grams away from the actual birth weights The standard error is 3.23 suggests that the RMSE value potentially deviates by approximately 3.23 units from the true error on average. Still, the Random Forest model excels in accurately predicting outcomes compared to others. Consequently, when prioritizing precise predictions, selecting **the Random Forest model using recipe 1 would be the most favorable option**.
...
One possible reason for this is that the variables used to predict the target variables may have not had a large influence on the target variable of interest. For instance, the Edinburgh Postnatal Depression Scale, household income, and the age of the mother may not have had any relationship with the baby's birth weight scientifically, thus the model may have not had good predictors to accurately predict the weight of the baby at birth. The original dataset used in this project only has 15 predictor variables; more number of biological predictors relevant to a baby's birth weight could lead to construction of a better performing model. In addition, there was little variance in the target variable compared to the number of observations. Because birth weight data has a small interquartile range compared to its median and range, it may have been harder for models to make accurate predictions. 

Another point of further investigation is that the RMSE of the model fitted to my entire training dataset is better than when the model was resampled (10 folds, 5 repeats). Considering the data variability, overfitting, and randomness, it is not uncommon for the RMSE of a model trained on the entire dataset to be better than that obtained from cross-validation. However, it is essential to reconsider the potential reasons behind the difference and to interpret the results cautiously with appropriate background context. Overall, further exploration and refinement could be performed as a next step to this project to unlock the full potential of predictive modeling in this context.
...
@fig-appendix-eda1 shows the distribution of Edinburgh Postnatal Depression Scale in the original scale and the square-root transformed scale.

```{r}
#| label: fig-appendix-eda1
#| fig-cap: "Edinburgh Postnatal Depression Scale Distribution"
#| echo: false
...
@fig-appendix-eda2 shows a scatterplot to examine the relationship between the Edinburgh Postnatal Depression Scale and Patient-Reported Outcomes Measurement Information System (PROMIS) on Anxiety of pregnant mothers. 
...
ggplot(pregnancy_train, aes(x = postnatal_depression, y = promis_anxiety)) +
  ...
  geom_smooth(method = "lm", se = FALSE, 
              color = "blue", linetype = 5) +
  theme_minimal() +
  labs(title = "Edinburgh Postnatal Depression Scale vs. PROMIS Anxiety Levels",
       x = "Edinburgh Postnatal Depression Scale",
       y = "PROMIS Anxiety")
```

## Appendix: RMSE by Model Parameters (Autoplots)

@fig-autoplot-en2 shows the RMSE by hyperparameters of the Elastic Net models using the customized recipe.

```{r}
#| label: fig-autoplot-en2
#| fig-cap: "RMSE by Hyperparameters of Elastic Net Models (recipe 2)"
#| echo: false

(lasso2_auto + ridge2_auto)
```

@fig-autoplot-knn shows the RMSE by hyperparameters of the K Nearest Neighbor model using the kitchen sink recipe. 

```{r}
#| label: fig-autoplot-knn
#| fig-cap: "RMSE by Hyperparameters of K Nearest Neighbor Model (recipe 1)"
#| echo: false

knn1_auto
```

@fig-autoplot-bt shows the RMSE by hyperparameters of the Boosted Tree model using the customized recipe. 

```{r}
#| label: fig-autoplot-bt
#| fig-cap: "RMSE by Hyperparameters of Boosted Tree Model (recipe 2)"
#| echo: false

bt2_auto
```

@fig-autoplot-rf shows the RMSE by hyperparameters of the Random Forest model using the customized recipe. 

```{r}
#| label: fig-autoplot-rf
#| fig-cap: "RMSE by Hyperparameters of Random Forest Model (recipe 2)"
#| echo: false
