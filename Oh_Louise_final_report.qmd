...
The COVID-19 pandemic was a substantial stressor, especially for pregnant individuals. The Mental Health In Pregnancy During COVID-19 Dataset[^1] is a dataset used in a research study aimed to understand the impact of COVID-19-related stresses on pregnant women and their infants and collected survey-based data across Canada as part of the Pregnancy during the COVID-19 Pandemic (PdP) project (Lebel et al., 2023)[^1]. This survey-based data allows for insights that can inform targeted interventions and support strategies to address the impact of COVID-19-related stresses on maternal and infant health.
...
-   Apply square root transformation to postnatal depression for normality.
    ...
    -   Before the predictors were normalized, the postnatal depression data was transformed for a normal distribution. This decision was based on a univariate analysis shown in @fig-appendix-eda1 (see Appendix: EDA). 
...
The lasso regression, ridge regression, k nearest neighbor, boosted tree, and random forest models had their parameters tuned. Sub-models and the tuning process for each model types were examined. First, a visual inspection of autoplots was done over each model using the better recipe. (See Appendix: RMSE by Model Parameters for autoplots of all models)

@fig-autoplot-en1 shows the RMSE by parameters for the elastic net models (Lasso and Ridge Regressions) using the kitchen sink recipe. 
...
@fig-autoplot-knn2 shows the RMSE by parameters for the K Nearest Neighbor models using the customized recipe. 
...
@fig-autoplot-tree1 shows the RMSE by parameters for the other Boosted Tree and Random Forest models using the kitchen sink recipe. 

```{r}
#| label: fig-autoplot-tree1
#| fig-cap: "RMSE by Hyperparameters of Tree-Based (recipe 1) Models"
#| echo: false

(bt1_auto / rf1_auto)
```

It is easier to view the best hyperparameters by RMSE for comparison on each of the objects containing the tuning information. @tbl-parameters shows the best hyperparameters of each model adjusted to and used to fit the data for both recipes. These tables were used after fitting each models to adjust the parameters based on the best performing submodel for each model type. 
...
Kalin, N. H. (2020). The critical relationship between anxiety and depression. American Journal of Psychiatry, 177(5), 365–367. (https://doi.org/10.1176/appi.ajp.2020.20030305)^[https://doi.org/10.1176/appi.ajp.2020.20030305]

Lebel, C., Tomfohr-Madsen, L., Giesbrecht, G., Lai, B. P. Y., Bagshawe, M., Freeman, M., Hapin, M. K., MacKinnon, A., Patel, P., van Sloten, M., & van de Wouw, M. (2023). Prenatal mental health data and birth outcomes in the pregnancy during the COVID-19 Pandemic dataset. Data in brief, 49, 109366. (https://doi.org/10.1016/j.dib.2023.109366)^[https://doi.org/10.1016/j.dib.2023.109366]

## Appendix: EDA (Exploratory Data Analysis)

@fig-appendix-eda1 shows the distribution of postnatal depression level in the original scale and the square-root transformed scale.
...
@fig-appendix-eda2 shows a scatterplot to examine the relationship between the postnatal depression and PROMIS anxiety levels of pregnant mothers. 
...
@fig-autoplot-lasso shows the RMSE by hyperparameters of the Lasso Regression Models by recipe.

```{r}
#| label: fig-autoplot-lasso
#| fig-cap: "RMSE by Hyperparameters of Lasso Regression Models"
#| echo: false

(lasso1_auto + lasso2_auto)
```

@fig-autoplot-ridge shows the RMSE by hyperparameters of the Ridge Regression Models by recipe. 

```{r}
#| label: fig-autoplot-ridge
#| fig-cap: "RMSE by Hyperparameters of Ridge Regression Models"
#| echo: false

(ridge1_auto + ridge2_auto) 
```

@fig-autoplot-knn shows the RMSE by hyperparameters of the K Nearest Neighbor Models by recipe. 

```{r}
#| label: fig-autoplot-knn
#| fig-cap: "RMSE by Hyperparameters of K Nearest Neighbor Models"
#| echo: false
#| out-width: "2"
#| out-height: "1"
...
@fig-autoplot-bt shows the RMSE by hyperparameters of the Boosted Tree models by recipe. 

```{r}
#| label: fig-autoplot-bt
#| fig-cap: "RMSE by Hyperparameters of Boosted Tree Models"
#| echo: false

(bt1_auto / bt2_auto)
```

@fig-autoplot-rf shows the RMSE by hyperparameters of the Random Forest models by recipe. 

```{r}
#| label: fig-autoplot-rf
#| fig-cap: "RMSE by Hyperparameters of Random Forest Models"
#| echo: false
